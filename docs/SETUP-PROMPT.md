...
**Q9:** "Would you like to install JaxTax — a browser-based dashboard for uploading receipts and invoices and reviewing them before they hit your tax records?

  JaxTax is a full web interface that runs on your machine at http://localhost:7331. You can:
  - Upload receipts and invoices from your browser (photo, PDF, or scan)
  - See AI extraction results — vendor, amount, date, category — before they're filed
  - Review, correct, and approve documents visually
  - Search and browse all your transactions

  Documents uploaded through JaxTax automatically drop into your ai-accountant inbox — no extra steps needed.

  Requires: Docker (just for the database). I'll check if it's installed and walk you through it if not.

  (yes / no — you can always install it later)"
→ Store as `WANTS_JAXTAX`
...
- JaxTax web dashboard: [yes — http://localhost:7331 / no]
...
const watcher = chokidar.watch(inboxPath, {
  ...
  persistent: true,
  ignoreInitial: true,
  depth: undefined,        // watch all subdirectories recursively — required for JaxTax uploads
                           // which are stored as inbox/receipts/{email}/{YYYY}/{MM}/file
  ignored: [/^\.|DS_Store|Thumbs\.db|\.tmp$|\.lock$|\.webp$/],
  // .webp files are preview thumbnails generated by JaxTax — ignore them
  awaitWriteFinish: {      // wait for file to be fully written before triggering
...
[If WANTS_JAXTAX = yes, also print:]

WEB DASHBOARD (JaxTax):
  Open in browser → http://localhost:7331
  Upload receipts, invoices, and documents through your browser.
  Uploaded files automatically flow into your ai-accountant inbox.

  pm2 logs jaxtax         — view JaxTax logs
  pm2 restart jaxtax      — restart the web dashboard
...
## Phase 11: JaxTax Web Dashboard

**Only run this phase if `WANTS_JAXTAX = yes`.**

JaxTax is a rebranded build of the open-source TaxHacker project (MIT licensed). It provides a browser-based upload and review interface that feeds directly into the ai-accountant inbox.
...
- **Mac:** "Please install Docker Desktop from https://docker.com/products/docker-desktop — it's free. Once installed and running, come back and I'll continue."
- **Windows:** "Please install Docker Desktop from https://docker.com/products/docker-desktop. Make sure to enable WSL2 integration during install."
- **Linux:** Run: `curl -fsSL https://get.docker.com | sh && sudo usermod -aG docker $USER` — then ask the user to log out and back in, and continue.

Wait for confirmation before proceeding. Docker must be running (not just installed).

### Step 2: Clone and Rebrand

```bash
git clone https://github.com/vas3k/TaxHacker.git [INSTALL_DIR]/jaxtax
cd [INSTALL_DIR]/jaxtax
```

**Rebranding — targeted replacements only.** Do not blanket-replace all occurrences (it would break npm package names and internal identifiers). Replace only display text and configuration:

1. In `package.json`: change `"name": "taxhacker"` → `"name": "jaxtax"`

2. In `app/layout.tsx` and any file containing `<title>` tags or `metadata.title`: replace `"TaxHacker"` → `"JaxTax"`

3. In `app/landing/page.tsx` (and any other marketing/landing pages): replace visible text `TaxHacker` → `JaxTax`

4. Create `public/logo/anthropic.svg` — a simple SVG containing the letter "A" in Anthropic's style, or a plain text SVG:
   ```svg
   <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text y=".9em" font-size="90">A</text></svg>
   ```

5. In any `.tsx` component that renders the app name as a string, replace `"TaxHacker"` → `"JaxTax"`.

Do NOT replace occurrences inside import paths, npm package names, CSS class names, or variable names.

### Step 3: Add Anthropic LangChain Provider

The user already has an Anthropic API key. Add Anthropic as a provider so JaxTax can use it for document extraction without requiring an additional OpenAI or Gemini key.
...
**Add to `lib/llm-providers.ts`** — insert this entry at the TOP of the `PROVIDERS` array so it appears first (it's the default for users who set up via this prompt):
```typescript
{
  key: "anthropic",
  ...
  apiDoc: "https://console.anthropic.com/",
...
**Find the file that implements the `requestLLM` function** (likely `ai/providers/llmProvider.ts` or similar). Add a case for `'anthropic'`:
```typescript
import { ChatAnthropic } from '@langchain/anthropic';

// Inside the provider switch/if block, add:
case 'anthropic': {
  const model = new ChatAnthropic({
    model: settings.anthropic_model_name || 'claude-haiku-4-5-20251001',
    apiKey: settings.anthropic_api_key,
  });
  // follow same pattern as existing OpenAI/Google/Mistral implementations
  break;
}
```

### Step 4: Create the Docker Database Configuration

Create `[INSTALL_DIR]/jaxtax/docker-compose.db.yml` — PostgreSQL only (JaxTax itself runs via PM2, not Docker):

```yaml
services:
  jaxtax-db:
    image: postgres:17-alpine
    container_name: jaxtax-db
    environment:
      - POSTGRES_USER=jaxtax
      - POSTGRES_PASSWORD=[generate a random 24-char password, store as JAXTAX_DB_PASSWORD]
      - POSTGRES_DB=jaxtax
    volumes:
      - jaxtax-pgdata:/var/lib/postgresql/data
    ports:
      - "5433:5432"
...
Note: using port 5433 (not 5432) to avoid conflicts if the user already has a local PostgreSQL.

### Step 5: Create Environment Configuration

Create `[INSTALL_DIR]/jaxtax/.env.local`:
...
# JaxTax uploads feed directly into the ai-accountant inbox
UPLOAD_PATH="[INSTALL_DIR]/inbox/receipts"

DATABASE_URL="postgresql://jaxtax:[JAXTAX_DB_PASSWORD]@localhost:5433/jaxtax"

# Anthropic — reuse the existing API key from the main setup
ANTHROPIC_API_KEY="[ANTHROPIC_API_KEY]"
ANTHROPIC_MODEL_NAME="claude-haiku-4-5-20251001"

# Auth — required by better-auth
BETTER_AUTH_SECRET="[generate a random 48-char alphanumeric string]"
NEXT_PUBLIC_APP_URL="http://localhost:7331"

# Email (optional — leave blank to disable)
RESEND_FROM_EMAIL="JaxTax <noreply@localhost>"
```

Do not write Stripe keys — they are not needed for self-hosted local use.
...
# Wait for PostgreSQL to be ready (poll until healthy)
until docker exec jaxtax-db pg_isready -U jaxtax; do sleep 1; done
...
If `npm run build` fails due to TypeScript errors introduced by the Anthropic provider addition, fix the errors before continuing — do not skip the build.

### Step 7: Add JaxTax to PM2

Add a second entry to `[INSTALL_DIR]/ecosystem.config.cjs`:

```javascript
// Add this to the apps array alongside the existing ai-accountant entry:
{
  name: 'jaxtax',
  script: 'node_modules/.bin/next',
  args: 'start -p 7331',
  cwd: '[INSTALL_DIR]/jaxtax',
  env: {
    NODE_ENV: 'production'
  },
  watch: false,
...
```bash
# Restart PM2 with updated config to pick up the jaxtax app
pm2 startOrReload [INSTALL_DIR]/ecosystem.config.cjs

# Wait 10 seconds for Next.js to start
```

Check `pm2 list` — both `ai-accountant` and `jaxtax` should show `online`.

Open `http://localhost:7331` in a browser (or tell the user to open it). They should see the JaxTax login screen.

**First-time account creation:** Since `DISABLE_SIGNUP=false`, the user can create their account on first visit. After they sign in, walk them through:
1. Go to Settings → AI Provider → select "Anthropic (Claude)" → the key is pre-filled from the environment
2. Upload a test receipt to confirm extraction works
3. Check that `[INSTALL_DIR]/inbox/receipts/` now contains the uploaded file

### Step 9: Update the Health Check

Update `src/scripts/health-check.ts` to also check JaxTax:
- Is the `jaxtax` PM2 process online?
- Is port 7331 responding to HTTP?
- Is the `jaxtax-db` Docker container running? (`docker inspect jaxtax-db --format '{{.State.Status}}'`)

Print result: `JaxTax: ✓ http://localhost:7331` or `JaxTax: ✗ not running — run: pm2 restart jaxtax`
...
- JaxTax: `docker-compose` not found → try `docker compose` (newer Docker versions use subcommand syntax)
- JaxTax: PostgreSQL container fails to start → check port 5433 is free: `lsof -i :5433`
- JaxTax: `npm run build` fails with TypeScript error in Anthropic provider → check `@langchain/anthropic` version is compatible with the installed `langchain` version; try pinning to `^0.3.0`
- JaxTax: Next.js start fails with "Module not found" → run `npx prisma generate` before `npm run build`
- JaxTax: Port 7331 already in use → change `PORT=7332` in `.env.local` and update the PM2 entry's `args` field
